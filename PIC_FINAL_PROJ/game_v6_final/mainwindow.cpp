...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
      ...
      //adding every page of the overall game to the stack widget
      ...
      stackWidget->addWidget(gameover); // final game over - index 6
      stackWidget->addWidget(lose_2); // lose game 1 - index 7
      stackWidget->addWidget(lose_3); // lose game 2 - index 8
      stackWidget->addWidget(won);  //win the game - index 9
      ...
      stackWidget->setCurrentIndex(13); //start at the main instructions page
      ...
      this->setWindowTitle("Arcade 10C");
...
void MainWindow::go_to_gametwo(){
    if(game_two->won)       //if game two is already won, go to win page
           stackWidget->setCurrentIndex(9);
    else {                  //if game two hasn't been beat yet, go to game
...
void MainWindow::go_to_gamethree(){
    game_three->game->p->timer->start(50);
...
void MainWindow::lose_gametwo(){
    ...
    if(live==1){    //if 1 lives left, remove live 2
        num_lives->layoutt->removeWidget((num_lives->live_2));
    }
    if(live==0){    //if 0 lives left, remove last live
        num_lives->layoutt->removeWidget((num_lives->live_3));
    }
    setStatusBar(sbar);
    if(live > 0)        //still have lives left, go to lose page
        stackWidget->setCurrentIndex(8);
    else {              //no lives left, go to game over page
        stackWidget->setCurrentIndex(6);
    }
}
...
void MainWindow::go_to_losepage(){  
    lose_life();
    game_three->game->p->setPos(900,game_three->game->p->pos().y());    //move player off screen
    if(live==2){        //if 2 lives left, remove live 1
        num_lives->layoutt->removeWidget((num_lives->live_1));
    }
    if(live==1){        //if 1 lives left, remove live 2
    ...
    setStatusBar(sbar);
    if(live > 0)    //still have lives left, go to you lose page
        stackWidget->setCurrentIndex(5);
    else {          //no lives left->game over page
        stackWidget->setCurrentIndex(6);
    }
}
...
void MainWindow::connect_buttons(){
    ...
    //memory management for game_two
    QObject::connect(game_two->Tic_Tac_Toe->board, SIGNAL(win()), game_two, SLOT(delete_1()));
    QObject::connect(game_two->Tic_Tac_Toe2->board, SIGNAL(win()), game_two, SLOT(delete_2()));
    QObject::connect(game_two->Tic_Tac_Toe3->board, SIGNAL(win()), game_two, SLOT(delete_3()));
    QObject::connect(game_two->Tic_Tac_Toe3->board, SIGNAL(lose()), game_two, SLOT(delete_3()));
...
void MainWindow::turn_off(){
    if(flag_volume) {   //clicked once, turn off music
        flag_volume = false;
        music->setVolume(0);
    } else {            //clicked again, turn music back on
        flag_volume = true;
        music->setVolume(50);
    }
}
