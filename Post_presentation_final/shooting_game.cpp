...
void shooting_game::update_scoreboard(){
    ...
    if(p->score==20){        //player meets criteria for winning score
        emit won_game();
        p->timer->stop();   //stop spawing enemies
    }
    ...
    if(p->lost){            //player has lost
        ...
        p->timer->stop();   //stop spawning enemies
    }
}
 ...
 * @return N/A
 */
bullet::bullet( player* player_1 ):owner(player_1)
{
 ...
 * @return N/A
 */
enemy::enemy(player* play): player1(play)
{
 ...
 * */
void enemy::move(){
    if(player1->delete_enemies){      //if delete_enemies is true ( when game lost) - remove enemy and delete
       scene()->removeItem(this);
       delete this;
       return;
    }
    else {                      //game is not lost
        setPos(x(),y()+5);
    }
    ...
    QList<QGraphicsItem*> colliding_items=this->collidingItems();
       for(int i = 0; i < colliding_items.size(); ++i){    //iterates through list of colliding items with enemy
           if(typeid(*(colliding_items[i])) == typeid(player))     //if item colliding with this enemy is a player, remove both from scene and free memory
           {
                   player1->lost= true;
                   scene()->removeItem(this);
                   player1->setPos(900,player1->pos().y());
                   //delete colliding_items[i];
                   delete this;
                   return;
           }
    ...
    if (pos().y() > 600){       //if enemy is off screen
            scene()->removeItem(this);
            delete this;
        }
}
