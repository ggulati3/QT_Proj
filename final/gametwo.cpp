...
void gametwo::reset()
{
    ...
    QObject::connect(Tic_Tac_Toe2->board, SIGNAL(lose()), this, SLOT(reset2()));
}
...
void Game::connections()
{
    for (int i=0; i< 10; i++)       //connect for all 9 box objects in the vector
    {
        ...
        //QObject::connect((*board)[i], SIGNAL(check()), board, SLOT(check_game()));

    }
    //QObject::connect(board, SIGNAL(check()), board, SLOT(change_status()));
    //QObject::connect(board, SIGNAL(check()), board, SLOT(check_game()));
}
...
/*void vec::wait_for_comp(){
    QTimer::singleShot(1200, this, SLOT(computer_play()));
}
*/
